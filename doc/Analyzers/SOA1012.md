...
Two or more scheduled jobs cannot have the same GUID assigned to them,
they must be unique for Optimizely to be able to tell them apart.
See documentation for SOA1008 for more information and important
details surrounding the GUID.

This analyzer only considers usages of `ScheduledPluginAttribute`
in the open project(s), not all GUIDs in general and not scheduled jobs
inside nuget packages.
