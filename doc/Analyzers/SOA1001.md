...
1. Add the `ContentType` attribute, the content type will show up
for editors in the CMS.
2. Add the `ContentType` attribute and set the `AvailableInEditMode`
...
other types I'll add the `ContentType` attribute and set the `AvailableInEditMode`
property to `false`.
...
The `GroupName` parameter can be used to group related content types

Define the name and description of the content type using the
[XML language files](https://docs.developers.optimizely.com/content-cloud/v12.0.0-content-cloud/docs/localizing-the-user-interface)
to support localizing them to different languages, instead of using the properties on the attribute.

You have many more options than just `AvailableInEditMode` when it comes to controlling the creation of new instances of
...
in the previous paragraph at any time. Use this to, for example, hide special
types like the start page once the site is set up and running, making it easier
for editors to find the type they want in the list of available types.
