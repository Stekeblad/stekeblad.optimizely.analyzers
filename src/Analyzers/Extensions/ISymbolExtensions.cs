...
namespace Stekeblad.Optimizely.Analyzers.Extensions
{
	...
	{
		/// <summary>
		/// Checks if <c> analyzedSymbol</c> is decorated with the attribute <c> attributeToFind</c> or any attribute deriving
		/// from <c> attributeToFind</c>. Base types of <c> analyzedSymbol</c> is NOT examined when searching for attributes.
		/// </summary>
		...
		/// <summary>
		/// Checks if <paramref name="analyzedSymbol"/> is decorated with the attribute <paramref name="attributeToFind"/> or
		/// any attribute deriving from <c> attributeToFind</c>. Base types of <c> analyzedSymbol</c> is NOT examined when
		/// searching for attributes. To not get derived attributes, use
		/// <see cref="TryGetAttribute(ISymbol, ITypeSymbol, out AttributeData)" />
		/// </summary>
		...
		/// <param name="foundAttribute">
		/// Will be set to the first found matching attribute, or null if no matching attribute was found.
		/// </param>
		/// <returns>true if the attribute was found on the analyzed symbol, false otherwise</returns>
		...
		/// <summary>
		/// Checks if <paramref name="analyzedSymbol"/> is decorated with the attribute <paramref name="attributeToFind"/> If
		/// you want to include derived attributes aswell, use
		/// <see cref="TryGetAttributeDerivedFrom(ISymbol, ITypeSymbol, out AttributeData)" />
		/// </summary>
		/// <param name="analyzedSymbol">The symbol that may be decorated with <paramref name="attributeToFind"/></param>
		/// <param name="attributeToFind">The attribute to test for</param>
		/// <param name="foundAttribute">
		...
		/// <summary>
		/// Checks if <paramref name="analyzedSymbol"/> is decorated with the attribute <paramref name="attributeToFind"/> or
		/// any attribute deriving from it. Base types of <paramref name="analyzedSymbol"/> is NOT examined when searching for
		/// attributes. To not get derived attributes, use
		/// <see cref="TryGetAttributesOfType(ISymbol, ITypeSymbol, out List{AttributeData})" />
		/// </summary>
		/// <param name="analyzedSymbol">
		/// The symbol that may be decorated with <paramref name="attributeToFind"/> or a deriving attribute
		/// </param>
		/// <param name="attributeToFind">The attribute to test for</param>
		...
		/// <summary>
		/// Checks if <paramref name="analyzedSymbol"/> is decorated with the attribute<paramref name="attributeToFind"/> If
		/// you want to include derived attributes as well, use
		/// <see cref="TryGetAttributeDerivedFrom(ISymbol, ITypeSymbol, out AttributeData)" />
		/// </summary>
		/// <param name="analyzedSymbol">The symbol that may be decorated with <paramref name="attributeToFind"/></param>
		/// <param name="attributeToFind">The attribute to test for</param>
		...
		/// <summary>
		/// Test if a symbol is a valid Optimizely content property. Tests include if the symbol is a property, how it is
		/// defined and that it's containing type is derived from EPiServer.Core.ContentData
		/// </summary>
		/// <param name="symbol">Symbol to test</param>
		/// <param name="compilation">
		/// The current compilation information, to locate and test presence of Optimizely types
		/// </param>
		/// <returns>
		/// True if symbol meets all criteria for a content property (except for being virtual, that is
		/// <see cref="Analyzers.Content.ContentPropertyMustBeVirtualAnalyzer.AnalyzeNamedTypeSymbol"> covered by
		/// SOA1003</see>), false otherwise
		/// </returns>
		public static bool IsOptiContentProperty(this ISymbol symbol, Compilation compilation)
		{
			...
			if (prop.DeclaredAccessibility == Accessibility.Public
				...
				&& prop.GetMethod.DeclaredAccessibility == Accessibility.Public) // getter must be public
			{
